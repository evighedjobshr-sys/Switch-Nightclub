const $ = (s, p = document) => p.querySelector(s);
const $$ = (s, p = document) => [...p.querySelectorAll(s)];

const prog = $('#prog');
const nav = $('#nav');
function updateProgress(){
  if(!prog) return;
  const h = document.documentElement;
  const sc = h.scrollTop / Math.max(1, h.scrollHeight - innerHeight);
  prog.style.transform = `scaleX(${sc})`;
}
window.addEventListener('scroll', () => {
  updateProgress();
  if(nav) nav.classList.toggle('on', window.scrollY > 24);
}, {passive:true});
updateProgress();
if(nav) nav.classList.toggle('on', window.scrollY > 24 || document.body.dataset.page !== 'home');

const cursor = $('#cursor');
if(cursor && window.matchMedia('(pointer:fine)').matches){
  window.addEventListener('mousemove', e => {
    cursor.style.transform = `translate(${e.clientX}px,${e.clientY}px)`;
  }, {passive:true});
  $$('a,button,.filter-btn,input,textarea,select,label').forEach(el => {
    el.addEventListener('mouseenter', () => document.body.classList.add('c-h'));
    el.addEventListener('mouseleave', () => document.body.classList.remove('c-h'));
  });
  $$('.gallery-item,.gal-card,.preview-image,.hero-image,.event-image,.gallery-thumb,.insta-card').forEach(el => {
    el.addEventListener('mouseenter', () => document.body.classList.add('c-img'));
    el.addEventListener('mouseleave', () => document.body.classList.remove('c-img'));
  });
}

const navToggle = $('#navToggle');
const mobileMenu = $('#mobileMenu');
if(navToggle && mobileMenu){
  const setMenu = open => {
    mobileMenu.classList.toggle('open', open);
    navToggle.classList.toggle('active', open);
    navToggle.setAttribute('aria-expanded', String(open));
    mobileMenu.setAttribute('aria-hidden', String(!open));
    document.body.classList.toggle('menu-open', open);
  };
  navToggle.addEventListener('click', () => setMenu(!mobileMenu.classList.contains('open')));
  $$('a', mobileMenu).forEach(a => a.addEventListener('click', () => setMenu(false)));
  window.addEventListener('keydown', e => { if(e.key === 'Escape') setMenu(false); });
}

$$('a[href^="#"]').forEach(a => {
  a.addEventListener('click', e => {
    const href = a.getAttribute('href');
    if(!href || href === '#') return;
    const target = $(href);
    if(target){
      e.preventDefault();
      target.scrollIntoView({behavior:'smooth'});
    }
  });
});

const srObs = new IntersectionObserver(entries => {
  entries.forEach(e => {
    if(e.isIntersecting){
      e.target.classList.add('in');
      srObs.unobserve(e.target);
    }
  });
}, {threshold:.12});
$$('.sr,.clip-line').forEach(el => srObs.observe(el));

// intro canvas/home only
const ic = $('#introCanvas');
const introWrap = $('#intro-wrap');
const titleWrap = $('#introTitle');
if(ic && introWrap && titleWrap){
  const ict = ic.getContext('2d');
  const introTag = $('#introTag');
  const introSub = $('#introSub');
  const scrollInd = $('#scrollInd');
  const topLine = $('.top-clip .title-line', titleWrap);
  const botLine = $('.bot-clip .title-line', titleWrap);
  const IW = [
    {f:.009,a:90,s:.028,p:0,c:'rgba(255,45,120,0.28)',lw:2,y:.72},
    {f:.013,a:70,s:.036,p:1.8,c:'rgba(0,240,255,0.24)',lw:2,y:.68},
    {f:.007,a:100,s:.022,p:3.5,c:'rgba(185,79,255,0.2)',lw:1.5,y:.76},
    {f:.016,a:55,s:.044,p:.9,c:'rgba(0,240,255,0.16)',lw:1.5,y:.56},
    {f:.02,a:45,s:.05,p:2.5,c:'rgba(255,45,120,0.18)',lw:1.5,y:.52},
    {f:.011,a:65,s:.032,p:4.8,c:'rgba(201,168,76,0.22)',lw:2,y:.62},
    {f:.022,a:38,s:.055,p:1.6,c:'rgba(185,79,255,0.14)',lw:1,y:.59},
    {f:.028,a:28,s:.065,p:.4,c:'rgba(0,240,255,0.12)',lw:1,y:.44},
    {f:.035,a:20,s:.08,p:2.1,c:'rgba(255,45,120,0.1)',lw:1,y:.38},
    {f:.018,a:35,s:.045,p:5.0,c:'rgba(201,168,76,0.12)',lw:1,y:.48},
    {f:.042,a:12,s:.1,p:.6,c:'rgba(185,79,255,0.08)',lw:.8,y:.32},
    {f:.05,a:8,s:.12,p:3.7,c:'rgba(0,240,255,0.07)',lw:.8,y:.28}
  ];
  let speedBoost = 1;
  let iT = 0, iRaf;
  function sizeIC(){ ic.width = innerWidth; ic.height = innerHeight; }
  function drawIntro(){
    ict.clearRect(0,0,ic.width,ic.height);
    iT += .022 * speedBoost;
    IW.forEach(w => {
      ict.beginPath();
      const by = ic.height * w.y;
      for(let x = 0; x <= ic.width; x += 3){
        const y = by + Math.sin(x*w.f + iT*w.s + w.p) * w.a + Math.sin(x*w.f*1.9 + iT*w.s*.6 + w.p) * (w.a*.38) + Math.sin(x*w.f*.55 + iT*w.s*1.4 + w.p) * (w.a*.18);
        x === 0 ? ict.moveTo(x, y) : ict.lineTo(x, y);
      }
      ict.strokeStyle = w.c;
      ict.lineWidth = w.lw;
      ict.stroke();
    });
    iRaf = requestAnimationFrame(drawIntro);
  }
  function onIntroScroll(){
    const rect = introWrap.getBoundingClientRect();
    const total = introWrap.offsetHeight - innerHeight;
    const p = Math.max(0, Math.min(1, -rect.top / Math.max(1, total)));
    const SPLIT_END=.38, SPEED_END=.65, FADE_END=.88;
    if(p <= SPLIT_END){
      const t = p / SPLIT_END;
      const move = t * 90;
      const fade = 1 - Math.pow(t, 1.2);
      topLine.style.transform = `translateY(-${move}px)`;
      botLine.style.transform = `translateY(${move}px)`;
      titleWrap.style.opacity = fade;
      titleWrap.style.filter = `blur(${t*8}px)`;
      const fadeOther = 1 - Math.min(1, t*2.5);
      if(introTag) introTag.style.opacity = fadeOther;
      if(introSub) introSub.style.opacity = fadeOther;
      if(scrollInd) scrollInd.style.opacity = 1 - Math.min(1, t*4);
    } else {
      titleWrap.style.opacity = 0;
      if(introTag) introTag.style.opacity = 0;
      if(introSub) introSub.style.opacity = 0;
      if(scrollInd) scrollInd.style.opacity = 0;
    }
    if(p > SPLIT_END && p <= SPEED_END){
      const t = (p - SPLIT_END) / (SPEED_END - SPLIT_END);
      speedBoost = 1 + t * 14;
    } else if(p <= SPLIT_END) speedBoost = 1;
    if(p > SPEED_END){
      const t = (p - SPEED_END) / (FADE_END - SPEED_END);
      ic.style.opacity = Math.max(0, 1 - t);
      speedBoost = 15;
    }
    if(p >= FADE_END){
      cancelAnimationFrame(iRaf); iRaf = null;
    } else if(!iRaf) drawIntro();
  }
  sizeIC();
  drawIntro();
  onIntroScroll();
  window.addEventListener('scroll', onIntroScroll, {passive:true});
  window.addEventListener('resize', sizeIC, {passive:true});
}

// accent sea canvas where present
const sc = $('#seaCanvas');
if(sc){
  const sct = sc.getContext('2d');
  const SW = [
    {f:.005,a:120,s:.012,p:0,c:'rgba(0,240,255,0.22)',lw:2.5,y:.88},
    {f:.007,a:100,s:.015,p:1.8,c:'rgba(255,45,120,0.24)',lw:2,y:.84},
    {f:.006,a:110,s:.011,p:3.5,c:'rgba(185,79,255,0.18)',lw:2,y:.92},
    {f:.01,a:80,s:.018,p:.9,c:'rgba(0,240,255,0.18)',lw:2,y:.72},
    {f:.013,a:65,s:.022,p:2.5,c:'rgba(255,45,120,0.2)',lw:1.5,y:.68},
    {f:.009,a:85,s:.016,p:4.8,c:'rgba(201,168,76,0.22)',lw:2,y:.78},
    {f:.018,a:48,s:.03,p:.3,c:'rgba(0,240,255,0.14)',lw:1.5,y:.60},
    {f:.022,a:40,s:.036,p:2.1,c:'rgba(255,45,120,0.14)',lw:1.5,y:.55},
    {f:.04,a:14,s:.08,p:.5,c:'rgba(201,168,76,0.12)',lw:1,y:.36}
  ];
  let sT = 0, seaRaf;
  function sizeSea(){ sc.width = sc.parentElement.offsetWidth; sc.height = sc.parentElement.offsetHeight; }
  function drawSea(){
    sct.clearRect(0,0,sc.width,sc.height);
    sT += .018;
    SW.forEach(w => {
      sct.beginPath();
      const by = sc.height * w.y;
      for(let x = 0; x <= sc.width; x += 4){
        const y = by + Math.sin(x*w.f + sT*w.s + w.p) * w.a + Math.sin(x*w.f*2.1 + sT*w.s*.7 + w.p) * (w.a*.42) + Math.sin(x*w.f*.5 + sT*w.s*1.5 + w.p) * (w.a*.22);
        x === 0 ? sct.moveTo(x, y) : sct.lineTo(x, y);
      }
      sct.strokeStyle = w.c;
      sct.lineWidth = w.lw;
      sct.stroke();
    });
    seaRaf = requestAnimationFrame(drawSea);
  }
  sizeSea();
  const sec = sc.parentElement;
  const obs = new IntersectionObserver(entries => {
    if(entries[0].isIntersecting){ if(!seaRaf) drawSea(); }
    else { cancelAnimationFrame(seaRaf); seaRaf = null; }
  }, {threshold:.04});
  obs.observe(sec);
  window.addEventListener('resize', sizeSea, {passive:true});
}

$$('.acc-hd').forEach(hd => {
  hd.addEventListener('click', () => {
    const row = hd.closest('.acc-row');
    const was = row.classList.contains('open');
    $$('.acc-row').forEach(r => r.classList.remove('open'));
    if(!was) row.classList.add('open');
  });
});
$('.acc-row')?.classList.add('open');

if(window.matchMedia('(pointer:fine)').matches){
  $$('.btn-g,.btn-o,.nav-cta,.sticky-book').forEach(btn => {
    btn.addEventListener('mousemove', e => {
      const r = btn.getBoundingClientRect();
      const x = e.clientX - r.left - r.width/2;
      const y = e.clientY - r.top - r.height/2;
      btn.style.transform = `translate(${x*.14}px,${y*.14}px)`;
    });
    btn.addEventListener('mouseleave', () => btn.style.transform = '');
  });
}

$$('.filter-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    const group = btn.closest('[data-filter-group]');
    if(!group) return;
    $$('.filter-btn', group).forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
    const value = btn.dataset.filter;
    const targetSel = group.dataset.filterGroup;
    $$(targetSel).forEach(item => {
      const show = value === 'all' || item.dataset.category?.split(' ').includes(value);
      item.classList.toggle('hide', !show);
    });
  });
});

const lightbox = $('#lightbox');
if(lightbox){
  const img = $('.lightbox-image', lightbox);
  const ttl = $('.lightbox-title', lightbox);
  const txt = $('.lightbox-text', lightbox);
  const close = $('.lightbox-close', lightbox);
  const setOpen = open => lightbox.classList.toggle('open', open);
  $$('.gallery-item').forEach(item => {
    item.addEventListener('click', () => {
      img.style.backgroundImage = item.dataset.image;
      ttl.textContent = item.dataset.title || 'Switch Night Club';
      txt.textContent = item.dataset.text || 'A premium Switch moment.';
      setOpen(true);
    });
  });
  close?.addEventListener('click', () => setOpen(false));
  lightbox.addEventListener('click', e => { if(e.target === lightbox) setOpen(false); });
  window.addEventListener('keydown', e => { if(e.key === 'Escape') setOpen(false); });
}
